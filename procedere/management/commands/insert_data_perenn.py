    ...
    def handle(self, *args, **options):
        ...
        def insert_produits():
            ...
            for entete, prod in models.produit.ENTETES_PRODUITS.items():
                ...
                new_prod = models.produit.Produit(
                    shortname=prod, entete=entete, name=name, timezone=tz)
        ...
        def insert_region(dicteur):
            ...
            models.granularite.Region.objects.bulk_create(
                liste_reg, batch_size=1000)

        def insert_dept(dicteur):
            liste_dept = []
            liste_reg = models.granularite.Region.objects.only(
                'id', 'insee', 'metrop')
            for d in dicteur:
            ...
            models.granularite.Dept.objects.bulk_create(
                liste_dept, batch_size=1000)

        def insert_grain(dicteur):
            liste_grain = []
            liste_dept = models.granularite.Dept.objects.only(
                'id', 'insee', 'metrop')
            for d in dicteur:
            ...
            models.granularite.Grain.objects.bulk_create(
                liste_grain, batch_size=5000)
        ...
        for td in options['todo']:
            if td not in allowed:
                raise CommandError(
                    'option "{}" not allowed (should be in {})'.format(td, allowed))
