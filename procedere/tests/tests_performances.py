...
class VitesseTraitementTestCase(TestCase):
    ...
    def insert_abo(self):
        ...
        models.etat.Etat_grain_produit.objects.bulk_update(
            list_etat, ['abo_array'], batch_size=5000)
    ...
    def create_fake_cdp(self, insee_list):
        ...
        with timing.Timer() as t:
            ...
            fakeCdp = {insee_list[i]: randList[i]
                       for i in range(len(insee_list))}
                ...
                if newt0 != 0:
                    for abo in dico_etat_abo[insee][0]:
                        dico_abo_etat0.setdefault(abo, []).append(
                            (dico_etat_abo[insee][2],
                             dico_etat_abo[insee][3], newt0)
                        )
    ...
    def test_can_treat_quickly(self):
        ...
        for j in range(self.cycles):
            with timing.Timer() as t:
                ...
                print(f'traitement')
                dico_etat_abo, dico_abo_etat0, alertAbo = self.applatissement(
                    fakeCdp)
