...
class TraitementCarto():
    ...
    def __init__(self, cdp):
        ...
        self.delta_t = int(param.Application.get_value('carto_delta_t'))
        self.cdp_regex = param.Application.get_value('regex_cdp')
    ...
    def process(self):
        ...
        try:
            ...
            cdp_reseau_hhmm = re.search(self.cdp_regex, original_cdp_filename,
                                        re.IGNORECASE).group(3)
            cdp_reseau_yyyymmdd = re.search(self.cdp_regex, original_cdp_filename,
                                            re.IGNORECASE).group(4)
            self.logger.info('Prefixe CDP : ' + cdp_prefix)

            # Load product from database
            for prod in models.produit.Produit.objects.filter(entete=cdp_prefix):
                p_name = prod.name.split()[0].lower() # apic/vf
                p_origin = prod.shortname[1:3].lower() # fr,oi,ag,nc
                product_for_file = Produit(p_name, p_origin, prod.entete, prod.timezone)
        ...
        try:
            ...
            for troncon, seuil_tr in self.cdp.seuils_troncons.items():
                try:
                    ...
                    self.logger.warning('Donnée non traitée: "' + troncon + ';'+seuil_tr+'"')
    ...
    def process_reseaux(self, produit):
        ...
        try:
            ...
            content = {'date':produit.timestamp, 'date_str':produit.timestamp_str,
                       'origin':produit.origin, 'n_c':str(produit.nb_gr_nc),
                       'n_1':str(produit.nb_gr_1), 'n_2':str(produit.nb_gr_2)}
            data_json['reseaux'].insert(0, content)

            # Sort per date [robustness, should not happen]
            data_json['reseaux'] = sorted(data_json['reseaux'],
            ...
            self.logger.info('Réseaux conservés jusqu\'à : ' \
                            + timestamp_delta_t + ' (' + str(self.delta_t) + 'H)')
    ...
    def handle_csv_row(self, type_data, elt, seuil, product_for_file, data_json):
        """
        Traite les lignes du fichier csv
        :param type_data: grain/troncon
        ...
        """
        # troncon
        if type_data == 'troncon':
            data_json['troncons'].append({'id':str(elt), 'c:':str(seuil)})
        # grain
        elif type_data == 'grain':
            data_json['communes'].append({'id':str(elt), 'c:':str(seuil)})
            self.compute_dept_list(elt, seuil)
            product_for_file.add_grain(seuil)

    @staticmethod
    def compute_date_fr(cdp_time_reseau, timezone):
        """
        Génération de la date au format 'Mercredi 05 Février à 17:30 Loc'
        en fonction de la timezone (UTC+x)
    ...
    def compute_dept_list(self, grain, seuil):
        """
        Génération de la liste des seuils (distincts)
        par département. ex: {"01":[-1,1],"02":[1,2]...}
...
class Produit():
    ...
    def add_grain(self, seuil):
        """ add grain, compute total per threshold """
        seuil = int(seuil)
        if seuil == -1:
            self.nb_gr_nc += 1
        elif seuil == 1:
