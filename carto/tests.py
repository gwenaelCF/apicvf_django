"""
Tests Unitaires Carto
"""
import json
import os
from django.conf import settings
from django.core import management
from django.core.files import File
from django.test import TestCase
from mflog import get_logger
from parameters import models as param
from carto import traitement_carto as tc
from procedere import models as pm
from helpers import timing

class CdpTestCase(TestCase):
    """
    Tests pour le processsus de génération des json pour la carto
    Usage:
    - 1. Le fichier testing_data.json doit être mis à jour pour associer le fichier cdp
    avec les fichiers json résultats,
    "cdp" pour le fichier json propre au cdp, "reseaux" pour le fichier avec la liste des reseaux
    "reseaux_diff" indique le fichier reseau qui servira de base de comparaison au moment
    de l'exécution du test
    Important : Les CDPs seront traités dans l'ordre du fichier testing_data.json
    - 2. Le fichier CDP à tester doit être placé sous carto/data/cdp_in
    - 3. Les fichiers json (à comparer avec ceux générés) doivent être placés sous carto/data/json
    """

    # testing data file
    testing_data_file = '/carto/data/testing_data.json'
    # input cdp folder
    cdp_in_folder = '/carto/data/cdp_in/'
    # expected json folder
    json_folder = '/carto/data/json/'

    @classmethod
    def setUpTestData(cls):
        '''
        Set up data for the whole TestCase
        '''
        with timing.Timer() as time:
            args = ['regles', 'produits', 'grains', 'params']
            management.call_command('insert_data_perenn', *args, verbosity=0)

        print(f'Base mise en place en {time.interval}')

    def setUp(self):
        '''
        Default setUp
        '''
        self.logger = get_logger("apicvf_django.carto.tests")
        # update json destination path for tests
        path_carto = param.System.get_value('chemin_carto')
        param.System.set_value('chemin_carto', path_carto + 'tests/')
        # get json (test) destination path
        self.dest_path = param.System.get_value('chemin_carto')
        # delete files
        self.purge_directory(self.dest_path)

    def test_cdp_main(self):
        '''
        Nominal tests
        '''
        self.logger.info(f'Début des tests')

        with open(settings.BASE_DIR + self.testing_data_file) as json_file:
            data_json = json.load(json_file)
            for cdp, results in data_json.items():
                # subTest permet d'exécuter l'intégralité des tests
                with self.subTest(cdp=cdp, results=results):
                    self.logger.info(f'Lancement du test pour le fichier {cdp}')
                    self.launch_cdp(cdp, results['cdp'],
                                    results['reseaux'], results['reseaux_diff'])

        self.logger.info(f'Fin des tests')

    def launch_cdp(self, cdp, json_result_cdp, json_result_reseau, json_expected_result_reseaux):
        '''
        Lancement du test pour un fichier CDP
        :param cdp: cdp file
        :param json_result_cdp: result cdp json file
        :param json_result_reseau: result cdp reseaux file
        '''
        # CDP input file
        cdp_in_file_path = settings.BASE_DIR + self.cdp_in_folder + cdp
        # JSON expected results
        json_expected_result_cdp_file_path = settings.BASE_DIR + self.json_folder + json_result_cdp
        json_expected_result_reseaux_file_path = settings.BASE_DIR + self.json_folder \
                                                    + json_expected_result_reseaux
        # JSON real result
        json_result_cdp_file_path = self.dest_path + json_result_cdp
        json_result_reseaux_file_path = self.dest_path + json_result_reseau

        # 1. Create Cdp object from cdp file
        cdp = self.create_cdp_object_from_file(cdp_in_file_path)

        # 2. Launch process carto
        status = self.launch_carto_process(cdp)

        # A. Check result
        self.assertTrue(status)

        # B. Check JSON CDP
        json_cdp = self.load_json(json_expected_result_cdp_file_path, json_result_cdp_file_path)
        self.assertEqual(json_cdp[0], json_cdp[1])

        # C. Check JSON reseaux
        json_reseaux = self.load_json(json_expected_result_reseaux_file_path,
                                      json_result_reseaux_file_path)
        self.assertEqual(json_reseaux[0], json_reseaux[1])

    @staticmethod
    def load_json(file1, file2):
        '''
        Chargement des fichiers json à comparer
        :param file1: chemin du premier fichier json
        :param file2: chemin second fichier json
        :return: Une liste avec les deux json à comparer
        '''
        with open(file1) as json_file:
            data_json_file1 = json.load(json_file)
        with open(file2) as json_file:
            data_json_file2 = json.load(json_file)
        return [data_json_file1, data_json_file2]

    @staticmethod
    def launch_carto_process(cdp):
        '''
        Lancement du processus carto
        :param cdp: Objet Cdp
        :return: True en cas de succès, False sinon
        '''
        carto_process = tc.TraitementCarto(cdp)
        return carto_process.process()

    @staticmethod
    def create_cdp_object_from_file(cdp_in_file_path):
        '''
        Instantiation de l'objet Cdp à parti du fichier
        :param cdp_in_file_path: Chemin du fichier Cdp
        :return: L'objet Cdp
        '''
        with open(cdp_in_file_path, 'rb') as cdpfile:
            cdp_file = File(cdpfile)
            cdp_file.name = os.path.basename(cdp_in_file_path)
            cdp = pm.produit.Cdp.create(cdp_file)
        return cdp

    def purge_directory(self, directory):
        '''
        Purge json result test directory
        '''
        self.logger.info(f'Purge du répertoire {directory}')
        for root, _, files in os.walk(directory):
            for fic in files:
                if fic.startswith('.git'):
                    continue
                file_path = os.path.join(root, fic)
                try:
                    self.logger.debug(f'Suppression du fichier {fic}')
                    os.unlink(file_path)
                except (PermissionError, FileNotFoundError, IsADirectoryError) as err:
                    self.logger.warning('Failed to delete %s. Reason: %s' % (file_path, err))
